...
/**
 * Function shape [AreInteractive] delegates to via [LocalAreInteractiveBinding] -- one binding per
 * platform, provided at the theme composition root ([com.arashrahimi46.iptv.ui.theme.AreIptvTheme]
 * for `:tv` wraps this with D-pad focus + the accent ring; `:mobile`'s theme root binds the plain
 * touch rendering). See [AreInteractiveSurface] for the shared glass rendering both delegate to.
 */
typealias AreInteractiveBinding = @Composable (
    onClick: () -> Unit,
    modifier: Modifier,
...
/**
 * Dispatches to the platform binding installed via [LocalAreInteractiveBinding] -- see
 * [AreInteractiveSurface] for the actual rendering contract these params describe. Kept as its own
 * composable (rather than every call site reading the CompositionLocal directly) so `:core`
 * components have one stable entry point regardless of which platform ends up rendering them.
 *
 * @param backgroundBrush Optional gradient FILL. Takes precedence over [backgroundColor] when set.
 * @param shadowElevation Sentinel "this surface should lift" value (dp); >0 draws a soft shadow.
...
fun AreInteractive(
    ...
    LocalAreInteractiveBinding.current(
        onClick,
        modifier,
        interactionSource,
        shape,
 ...
 * focus target, `DPAD_CENTER` key handling) or the accent focus RING -- those are layered on top by
 * the platform [AreInteractiveBinding] (TV's wraps this with D-pad focus + ring; `TvFocusable`
 * itself also calls this function directly, bypassing the binding, so it can keep its own
 * customizable ring color). Any caller that wants keyboard/D-pad focusability must attach its own
 * `.focusable(interactionSource)` (or rely on [combinedClickable]'s built-in focus registration) to
 * [modifier] before/around this composable -- [AreInteractiveSurface] only ever READS focus state
 * off [interactionSource], it never claims it.
 *
 * @param backgroundBrush Optional gradient FILL. Takes precedence over [backgroundColor] when set.
